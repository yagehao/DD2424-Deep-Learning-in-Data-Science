...
class DataProcess:
	...
	def read_file(self, filename, validation_filename):
		...
		Args:
			filename (str): training data file.
			validation_filename (str): validation data file.

		Returns:
			dataset (dict): a map container, indexes include
				- names_train
				- labels_train
				- names_validation
				- labels_validation
				- characters in my alphabet
				- length d: dimensionality of one-hot vector used to encode each character
				- K: number of classes
				- n_len: max length of name in dataset
    	"""
	...
	def create_one_hot(self, dataset):
		...
		Returns:
			dataset (dict): with three indexes added:
				- one_hot_label_train, one-hot encoding of training data.
				- one_hot_label_validation, one-hot encoding of validation data.
				- one_hot_friends, one-hot encoding of testing data.
		"""
...
class ConvNet:
	def __init__(self, dataset, dimensions=(50, 5, 50, 3), eta=0.01, rho=0.9, batch_size=100, n_ite=1000, n_update=200):
		...
		Args:
			...
			dimensions (tuple of 4 entries): (n1, k1, n2, k2) where
				n1, n2: the number of filters applied at layer 1 and layer 2,
				k1, k2: the width of the filters applied at layer 1 and layer 2.
			eta (float): learning rate.
			rho (float): momentum term.
			batch_size (int): batch size.
			n_ite (int): iterates.
			n_update (int): update steps.
		"""
		self.eta = eta
		self.rho = rho
	...
	def initialize(self):
		"""Generate random F1, F2, W by He Initialization.
				F1: Filter 1 matrix.
				F2: Filter 2 matrix.
				W: weight matirx.
		"""
...
# 0.3 Construct the convolution matrices
	def MFMatrix(self, F, n_len):
		"""construct convolutional matrix M^filter.

		Args:
			F (np.ndarray): 3D array containing the conv. filters at one layer.
			n_len (int): the number of cols of the input to that layer.

		Returns:
			MF (np.ndarray): MF matrix.
		"""
		(dd, k, nf) = F.shape
	...
	def MXMatrix(self, x_input, d, k, nf):
		"""construct convolutional matrix M^input."""
	...
	def fit(self, X, Y, Xv, Yv, graphics=False):
		...
		while not(stop):
			...
			for j in range(n_batch):
				...
				"""
				# gradient check
				# e = np.max(np.abs(x-y) / np.max(1e-8, np.abs(x) + np.abs(y)))
...
def main():
	...
	#print(dataset["one_hot_label_train"])
